function y=rev_conv(x,lag,amplitude,Fs);

% function y=rev_conv(x,lag,amplitude);
%
% convolution with reversed IR
...
% Now, deconvolve signal, Eq. 3, w/ time reversed filter
att=amplitude; % redefinition to make equation below shorter
dur=length(x); % signal length/duration in tabs

y=zeros(dur+5*lag,1);
for n=0:5 % filter order
    y(1+lag*n:lag*n+dur)=y(1+lag*n:lag*n+dur)+((-1).^(n+1)).*x.*att.^(6-n)*a(lag).^(5-n);
end

y=y(lag*6+1:length(y)); % time compensation